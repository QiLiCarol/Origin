...
import { 
  ...
  GripHorizontal, MoveDiagonal2, Maximize2, X
} from 'lucide-react';
...
const ChartWorkbench: React.FC<ChartWorkbenchProps> = ({ virtualTables, dashboards, activeDashboardId, onUpdateDashboard, lang }) => {
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    if (!activeOp || !canvasRef.current) return;
  ...
  }, [activeOp, activeDashboard, onUpdateDashboard]);
  ...
  return (
    <div className="flex flex-col gap-6 h-full relative">
      <div className="flex items-center justify-between">
        <div><h2 className="text-2xl font-bold text-slate-900">{activeDashboard.name}</h2></div>
        <div className="flex items-center gap-3">
          <div className="bg-white border border-slate-200 rounded-xl p-1.5 flex items-center gap-1">
          ...
          <button onClick={() => addWidget(ChartType.BAR)} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl font-bold text-sm">
      ...
      <div className="flex-1 min-h-0 relative">
        <div ref={canvasRef} className="dashboard-canvas p-4 rounded-2xl bg-white/50 border-2 border-dashed border-slate-200 min-h-full">
          {activeDashboard.widgets.map(widget => (
            <div key={widget.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col group relative" style={{ gridColumn: `${widget.layout.x + 1} / span ${widget.layout.w}`, gridRow: `${widget.layout.y + 1} / span ${widget.layout.h}` }}>
              <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between cursor-grab" onMouseDown={(e) => handleMouseDown(e, widget.id, 'move')}>
                <div className="flex items-center gap-2 truncate"><GripHorizontal className="w-4 h-4 text-slate-300" /><h4 className="text-xs font-black text-slate-800 uppercase truncate">{widget.title}</h4></div>
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => setInsightTarget(widget)} className="p-1.5 text-indigo-500 hover:bg-indigo-50 rounded-lg"><Sparkles className="w-3.5 h-3.5" /></button>
              ...
              <div className="flex-1 p-5 min-h-0 bg-white"><ChartRenderer widget={widget} virtualTables={virtualTables} lang={lang} /></div>
              <div className="absolute bottom-1 right-1 p-1 cursor-nwse-resize opacity-0 group-hover:opacity-100" onMouseDown={(e) => { e.stopPropagation(); handleMouseDown(e, widget.id, 'resize'); }}><MoveDiagonal2 className="w-4 h-4 text-slate-300" /></div>
            </div>
          ))}
        </div>
      </div>
      ...
      {isConfiguring && currentConfigWidget && (
        <div className="fixed inset-y-0 right-0 w-80 bg-white shadow-2xl border-l border-slate-200 z-[100] flex flex-col">
          <div className="p-6 border-b border-slate-200 flex items-center justify-between bg-slate-50/50">
            ...
            <button onClick={() => setIsConfiguring(null)} className="p-2 hover:bg-slate-200 rounded-full"><X className="w-5 h-5 text-slate-400" /></button>
          </div>
          <div className="flex-1 overflow-y-auto p-6 space-y-8">
            <div className="space-y-3"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t.componentName}</label><input type="text" value={currentConfigWidget.title} onChange={(e) => updateWidgetConfig(isConfiguring, { title: e.target.value })} className="w-full px-4 py-3 text-sm font-bold border-2 border-slate-100 rounded-xl bg-slate-50/50" /></div>
            <div className="space-y-3"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t.componentType}</label><div className="grid grid-cols-5 gap-1 bg-slate-100 p-1.5 rounded-xl">
                {[ { type: ChartType.BAR, icon: BarChart }, { type: ChartType.LINE, icon: LineChart }, { type: ChartType.PIE, icon: PieChart }, { type: ChartType.KPI, icon: CreditCard }, { type: ChartType.TABLE, icon: TableIcon } ].map(({ type, icon: Icon }) => (
                  <button key={type} onClick={() => updateWidgetConfig(isConfiguring, { type })} className={`p-2 flex items-center justify-center rounded-lg ${currentConfigWidget.type === type ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:bg-white'}`}><Icon className="w-4 h-4" /></button>
                ))}
              </div></div>
            <div className="space-y-3"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t.dataProvider}</label><select value={currentConfigWidget.dataSourceId} onChange={(e) => updateWidgetConfig(isConfiguring, { dataSourceId: e.target.value })} className="w-full px-4 py-3 text-sm font-bold border-2 border-slate-100 rounded-xl bg-slate-50/50"><option value="">None Selected</option>{virtualTables.map(vt => <option key={vt.id} value={vt.id}>{vt.name}</option>)}</select></div>
            {currentVt && (
              <><div className="space-y-3"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t.horizontalAxis}</label><select value={currentConfigWidget.config.xAxis} onChange={(e) => updateWidgetConfig(isConfiguring, { config: { ...currentConfigWidget.config, xAxis: e.target.value }})} className="w-full px-4 py-3 text-sm font-bold border-2 border-slate-100 rounded-xl bg-slate-50/50">{currentVt.fields.map(f => <option key={f} value={f}>{f}</option>)}</select></div>
                <div className="space-y-3"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t.metricValue}</label><select value={currentConfigWidget.config.yAxis} onChange={(e) => updateWidgetConfig(isConfiguring, { config: { ...currentConfigWidget.config, yAxis: e.target.value, valueKey: e.target.value }})} className="w-full px-4 py-3 text-sm font-bold border-2 border-slate-100 rounded-xl bg-slate-50/50">{currentVt.fields.map(f => <option key={f} value={f}>{f}</option>)}</select></div>
                <div className="space-y-3"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t.visualTheme}</label><div className="flex flex-wrap gap-2">{['#6366f1', '#10b981', '#f59e0b', '#ef4444', '#06b6d4'].map(color => (<button key={color} onClick={() => updateWidgetConfig(isConfiguring, { config: { ...currentConfigWidget.config, color }})} className={`w-10 h-10 rounded-xl border-2 ${currentConfigWidget.config.color === color ? 'border-slate-800 scale-110' : 'border-transparent opacity-60'}`} style={{ backgroundColor: color }} />))}</div></div>
              </>
            )}
          </div>
          <div className="p-6 border-t border-slate-200 bg-slate-50/50"><button onClick={() => setIsConfiguring(null)} className="w-full bg-slate-900 text-white py-4 rounded-xl font-black text-xs uppercase tracking-widest">{t.closePanel}</button></div>
        </div>
      )}

      {insightTarget && (
        <AIInsightDialog widget={insightTarget} dataSource={virtualTables.find(vt => vt.id === insightTarget.dataSourceId)} onClose={() => setInsightTarget(null)} onSaveAsCard={(insightText) => {
          const maxY = activeDashboard.widgets.reduce((acc, w) => Math.max(acc, w.layout.y + w.layout.h), 0);
